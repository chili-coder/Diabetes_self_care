...
public class AlarmPhysicalActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_alarm_physical);


        getSupportActionBar().setTitle("Time For Physical Activity ");



        Intent intent = getIntent();

        clockImageView = findViewById(R.id.clock_alarm_activity); //Creating the clock icon
        ...
        String medicineName = intent.getStringExtra("activityName"); // The drug name is shown here
        // The drug name is shown here
        textViewMedicineName = findViewById(R.id.text_view_medicine_name_alarm_activity);
        textViewMedicineName.setText(medicineName);

        textViewTime = findViewById(R.id.text_view_time_alarm_activity);
        ...
        DatabaseHelperPhysical databaseHelper = new DatabaseHelperPhysical(this);
        ...
        buttonDismiss = findViewById(R.id.button_dismiss_activity); // button to reject alarm
        ...
        buttonDismiss.setOnClickListener(v -> {
            ...
            if (databaseHelper.noOfDaysLeft(medicineName, nextAlarmTime) > 0)
                setAlarmPhysical(nextAlarmTime, medicineName);
            finish();
        });

        buttonSnooze = findViewById(R.id.button_snooze_activity); //snooze alarm
        buttonSnooze.setOnClickListener(v -> {
            ...
            setAlarmPhysical(nextAlarmTime, medicineName);//set the new alarm time and name back to the alarm setter.
            finish();
        });
        // The acknowledgment button is written between these two parts.
        buttonTake = findViewById(R.id.button_take_activity); //snooze the alarm for one day
