...
public class DatabaseHelperTobacco extends SQLiteOpenHelper {


    private static final String DB_NAME = "Tobaccoreminder";
    private static final int DB_VERSION = 1;
    private static final String DB_TABLE = "Tobaccos";
    ...
    public DatabaseHelperTobacco(Context context) {
    ...
    public void insertNewTabacco(String dietName, int day, int month,
                              int year, int noOfTimesPerDay, int totalDoses, String timings, String alertType) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(KEY_NAME, dietName);
        ...
        Log.i("Med-Dose DB Helper", dietName + day + month + year + noOfTimesPerDay + totalDoses + timings + alertType);
        db.close();
    }

    public void deleteTabacco(String dietName) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public List<Tobacco> getTabaccoList() {
        List<Tobacco> dietList = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.query(DB_TABLE, new String[]{KEY_NAME, KEY_TIMES_PER_DAY}, null, null, null, null, KEY_ID + " DESC");
        while (cursor.moveToNext()) {
            Tobacco homeItem = new Tobacco(cursor.getString(0), cursor.getString(1) + " times a day avoid tobacco");
