...
public class DatabaseHelperFollowup extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        String query = String.format("CREATE TABLE IF NOT EXISTS %s (" +
                ...
                "%s INTEGER NOT NULL, " +
                "%s TEXT NOT NULL, " +
                "%s TEXT NOT NULL);", DB_TABLE, KEY_ID, KEY_NAME,KEY_HOSPITAL,KEY_LOCATION, KEY_DAY, KEY_MONTH, KEY_YEAR,KEY_DATE, KEY_TIMES_PER_DAY, KEY_TOTAL_DOSES, KEY_TIMINGS, KEY_ALERT_TYPE);
    ...
    public void insertNewFollowup(String followupName,String hospital,String location, int day, int month,
                                  int year,String date, int noOfTimesPerDay, int totalDoses, String timings, String alertType) {
    ...
    public List<FollowupItem> getFollowupList() {
        ...
        Cursor cursor = db.query(DB_TABLE, new String[]{KEY_NAME,KEY_HOSPITAL,KEY_LOCATION, KEY_TIMES_PER_DAY}, null, null, null, null, KEY_ID+" DESC");
        while (cursor.moveToNext()) {
            FollowupItem homeItem = new FollowupItem(cursor.getString(0) , cursor.getString(1) ,cursor.getString(2));
